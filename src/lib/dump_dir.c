...
int dd_copy_file(struct dump_dir *dd, const char *name, const char *path)
{
    char *dest = concat_path_file(dd->dd_dirname, name);

    log_debug("copying '%s' to '%s'", path, dest);

    off_t copied = copy_file(path, dest, DEFAULT_DUMP_DIR_MODE | S_IROTH);
    if (copied < 0)
        error_msg("Can't copy %s to %s", path, dest);
    else
...
int dd_copy_file_unpack(struct dump_dir *dd, const char *name, const char *path)
{
    char *dest = concat_path_file(dd->dd_dirname, name);

    log_debug("unpacking '%s' to '%s'", path, dest);

    off_t copied = decompress_file(path, dest, DEFAULT_DUMP_DIR_MODE | S_IROTH);
    if (copied != 0)
        error_msg("Can't copy %s to %s", path, dest);
    else
